...
class UNet(nn.Module):
    def __init__(self, in_ch=1, ngf=16):
        ...
        self.layer9m = sub_deconv(ngf*8, ngf*8, mid=True)
        self.layer10m = sub_deconv(ngf*16, ngf*8)
        self.layer11m = sub_deconv(ngf*16, ngf*8)
        self.layer12m = sub_deconv(ngf*16, ngf*8)
        self.layer13m = sub_deconv(ngf*16, ngf*4)
        self.layer14m = sub_deconv(ngf*8, ngf*2)
        self.layer15m = sub_deconv(ngf*4, ngf)
        self.layer16m = nn.ConvTranspose2d(ngf*2, 1, kernel_size=5, stride=2, padding=2, output_padding=1, bias=False)
    ...
    def forward(self, cover):
        ...
        x_m = self.layer9m(x8, x8)
        x_m = self.dropout(x_m)
        x_m = self.layer10m(x_m, torch.cat([x_m, x7], dim=1))
        x_m = self.dropout(x_m)
        x_m = self.layer11m(x_m, torch.cat([x_m, x6], dim=1))
        x_m = self.dropout(x_m)
        x_m = self.layer12m(x_m, torch.cat([x_m, x5], dim=1))
        x_m = self.layer13m(x_m, torch.cat([x_m, x4], dim=1))
        x_m = self.layer14m(x_m, torch.cat([x_m, x3], dim=1))
        x_m = self.layer15m(x_m, torch.cat([x_m, x2], dim=1))
        x_m = self.layer16m(torch.cat([x_m, x1], dim=1))
        x_m = torch.sigmoid(x_m) - 0.5
        p_m = torch.relu(x_m)
